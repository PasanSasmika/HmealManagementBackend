...
export const addVisitorBooking = async (req: any, res: Response): Promise<void> => {
  try {
    ...
    // mealTypes is now an array, e.g., ['breakfast', 'lunch']
    ...
    // 2. Loop through selected meals and prepare data
...
export const issueVisitorMeal = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
